...
BREW_COMMON="act ansible bash bash-completion bat checkmake colordiff coreutils cowsay diff-so-fancy dos2unix drawio ffmpeg figlet findutils fontconfig fortune fzf gawk git gitui gnu-sed grep htop ipcalc iterm2 jq keycastr lynx md5sha1sum netcat pngcheck pre-commit python3 ripgrep sizeup socat tmux tree vagrant watch wget yamllint yq"
